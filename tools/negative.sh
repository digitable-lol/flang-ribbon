...
#   7. сверка — на НЕТРОНУТОЙ копии она зелёная, а перевранная арифметика
#      роняет её И НАЗВАНА расхождением; без первой половины красное на
#      порче не значило бы ничего;
...
#
# СНАЧАЛА — ЧТО НА НЕТРОНУТОЙ КОПИИ СВЕРКА ЗЕЛЁНАЯ, и только потом порча. Тот
# же порядок, что в восьмом контроле ниже, и по той же причине: красное на
# порче не значит ничего, пока не показано, что без порчи зелено. Сверка умеет
# краснеть не только расхождением — ей может быть просто нечем сверять
# (эталона нет, сети нет, эмиссия отказала), и такое красное к перевранной
# арифметике отношения не имеет.
#
# ЭТО НЕ УМОЗРЕНИЕ. Ствол 2026-09-05 (прогон 33967669223) покраснел ровно так:
# шаг конвейера завёл `DIGITWM=/tmp/digitwm` с master-клоном эталона, сверка
# вышла кодом 1 на отпечатке `ribbon.c`, а контроль умел сказать об этом лишь
# «покраснела, но расхождения не назвала» — жалоба верная, причина не названа.
# Теперь она называется здесь целиком, словами самой сверки.
if [ "${RIBBON_SKIP_COMPARE:-}" = "1" ]; then
  ...
  if ! code_of sh "$work/tree/tools/compare.sh"; then
    err "сверка красная на НЕТРОНУТОЙ копии — перевранная арифметика тут ни при чём."
    err "Вот её собственная жалоба:"
    sed 's/^/    /' "$work/out.txt" >&2
    fail "сверке нечем сверять; красное на порче ничего бы не доказало"
  fi
  ok "7. сверка: на нетронутой копии зелёная — значит, красное ниже от порчи"
  ...
  grep -q "РАСХОЖДЕНИЕ" "$work/out.txt" ||
    fail "сверка покраснела, но ни одного расхождения не назвала: $(cut -c1-400 "$work/out.txt")"
  ok "   перевранная арифметика роняет прогон против эталона, и названа расхождением"
fi
